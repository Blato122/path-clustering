...
def generate_csv_routes(name: str, net_dir: Path, path_gen_kwargs: dict, run_dir: Path) -> None:
    """
...
def generate_feature_file(name: str, net_dir: Path, osm_dir: Path, run_dir: Path) -> None:
    ...
    # OSM features not really used anymore - .osm not required
...
def enrich_routes(name: str, run_dir: Path) -> None:
    ...
    out_path = run_dir / f"{name}_routes_enriched.csv"
    ...
    out_path = run_dir / f"{name}_ranking_matrix.csv"
...
def main():
    ...
    parser.add_argument(
        "--run-name", "-r",
        required=True,
        help="Name for this run's output folder (e.g. ingolstadt_1). All results saved flat inside results/<run-name>/."
    )
    ...
    path_gen_kwargs = {
        "verbose" : True,
        "number_of_paths" : 12, # 12/4,
        "beta" : -1, #-0.5/-5,
        "weight" : "time",
        "num_samples" : 5000, #10000/10,
    ...
    for d in data_dir.iterdir():
        ...
        if names and name.lower() not in names:
            continue
        if args.routes: generate_csv_routes(name, data_dir, path_gen_kwargs, run_dir) # generates routes
        if args.features: generate_feature_file(name, data_dir, osm_dir, run_dir) # generates merged_edges
